...
Assert 'Node detects expiry during apply' ($apply -match 'const expired = reconciliation.expiresAt <= Math.floor\(Date.now\(\) / 1000\)' -and $apply -match 'summary.ok = summary.ok && !expired;' -and $apply -match 'process.exit\(writes.failed \|\| deletes.failed \|\| expired \? 3 : 0\)')
...
Assert 'Cosmos transport has a bounded timeout and no long retries' ($wiring -match 'requestTimeout: 2500\b' -and $wiring -match 'maxRetryAttemptCount: 0\b')
