...
import { reportRedactions, redactReportText, redactPortalPage, validateCaptureProfile } from './lib/chargeback-redaction.mjs';
...
try {
  ...
  for (const step of steps.filter(item => !only || only.includes(item.id))) {
    ...
    await page.waitForFunction(name => document.body.innerText.includes(name) && document.body.innerText.includes('Subscription ID'), step.item.name, { timeout: 30000 }).catch(() => {});
    ...
    if (step.nav) {
      ...
      text = await checkAuth();
      if (step.then) {
        const control = await findControl(page, step.then);
        if (!control) throw new Error(`Missing live control: ${step.then}`);
        await control.click();
        await page.waitForTimeout(6500);
        text = await checkAuth();
      }
