...
| AUM terminal (`aum`, legacy `claude-finops`) | Keyboard-first dashboard and commands over one engine: token and USD budgets, observed people, directory developer add/remove, governance, usage, trends, requests, reports | Administrators (Direct); administrators, viewers and scoped managers (through the AUM service or Turnstile); developer add/remove requires admin Graph rights | Direct: token and USD budgets through the scripts and USD reconciler when the gateway owns them. AUM service: advertised scoped writes. Turnstile: token budgets only until it exposes a real USD source; Entra membership remains AUM/script/portal | None: runs on the operator's machine | **$0** |
