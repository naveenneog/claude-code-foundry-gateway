...
| AUM terminal (`aum`) | Keyboard-first dashboard and commands over one engine: budgets, observed people, directory developer add/remove, governance, usage, trends, requests, reports | Administrators (Direct); administrators, viewers and scoped managers (through Turnstile); developer add/remove requires admin Graph rights | Direct: yes, through the scripts. Through Turnstile: Turnstile's own rules; Entra membership remains AUM/script/portal | None: runs on the operator's machine | **$0** |
