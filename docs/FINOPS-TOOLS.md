...
| AUM (`aum`, legacy `claude-finops`) | Keyboard-first dashboard and commands over one engine: token and USD budgets, people, governance, usage, trends, requests, reports | Administrators (Direct); administrators, viewers and scoped managers (through the AUM service or Turnstile) | Direct: token and USD budgets through the scripts and reconciler when the gateway owns them. AUM service: advertised scoped writes. Turnstile: token budgets only until it exposes a real USD source | None: runs on the operator's machine | **$0** |
